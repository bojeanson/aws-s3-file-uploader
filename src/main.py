...
from .DirectoryUploader import DirectoryUploader
    ...
    while True:
        ...
        try:
            du = DirectoryUploader(
                pathname=pathname, bucket_name=bucket_name, bucket_path=bucket_path, interval=interval, logger=logger
            )
            await du.Run()
        except Exception:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 6:
        ...
        logger.info(
            f"File uploader started with; pathname={pathname}, bucket_name={bucket_name}, bucket_path={bucket_path}, interval={interval}"
        )
